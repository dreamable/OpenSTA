...
static double gateModelRd(const LibertyCell *cell,const GateTableModel *gate_model,const RiseFall *rf,
	    double in_slew,double c2,double c1,float related_out_cap,
	    const Pvt *pvt,bool pocv_enabled) {
  float cap1 = c1 + c2;
  float cap2 = cap1 + 1e-15;
  ArcDelay d1, d2;
  Slew s1, s2;
  gate_model->gateDelay(cell, pvt, in_slew, cap1, related_out_cap, pocv_enabled, d1, s1);
  gate_model->gateDelay(cell, pvt, in_slew, cap2, related_out_cap, pocv_enabled, d2, s2);
...
void DmpAlg::findDriverParams(double ceff) {
  if (nr_order_ == 3)
    x_[DmpParam::ceff] = ceff;
  double t_vth, t_vl, slew;
  gateDelays(ceff, t_vth, t_vl, slew);
  double dt = slew / (vh_ - vl_);   // Scale slew to 0-100%
  double t0 = t_vth - vth_ * dt + log(1.0 - vth_) * rd_ * ceff;
  x_[DmpParam::dt] = dt;
  x_[DmpParam::t0] = t0;
...
static void evalDmpEqnsState(void *state){
...
void DmpAlg::gateDelays(double ceff,double &t_vth,double &t_vl,double &slew){
  ...
  slew = table_slew * slew_derate_;  // Convert reported/table slew to measured slew.
  t_vl = t_vth - slew * (vth_ - vl_) / (vh_ - vl_);
}

void DmpAlg::gateCapDelaySlew(double ceff,double &delay,double &slew){
...
// double DmpAlg::y(double t, double t0, double dt, double cl){
//   double t1 = t - t0;
//   if (t1 <= 0.0)
//     return 0.0;
//   else if (t1 <= dt)
...
// double DmpAlg::y0(double t, double cl){
//   return t - rd_ * cl * (1.0 - exp2(-t / (rd_ * cl)));
// }

// void DmpAlg::dy(double t, double t0, double dt, double cl,
...
// double DmpAlg::y0dt(double t,double cl){
//   return 1.0 - exp2(-t / (rd_ * cl));
// }

// double DmpAlg::y0dcl(double t,double cl) {
...
double DmpAlg::y0(double t, double cl){
  return t>0 ? (t - rd_ * cl * (1.0 - exp2(-t / (rd_ * cl)))) : 0;
}
...
double DmpAlg::y0dt(double t,double cl){
  return t>0 ? (1.0 - exp2(-t / (rd_ * cl))) : 0;
}

double DmpAlg::y0dcl(double t,double cl) {
  return  t > 0 ? rd_ * ((1.0 + t / (rd_ * cl)) * exp2(-t / (rd_ * cl)) - 1) ：0;
}


void DmpAlg::findDriverDelaySlew(double &delay,double &slew){
...
// TODO: What's the value for elmore? Rpi*C1?
void DmpAlg::loadDelaySlew(const Pin *,double elmore, ArcDelay &delay, Slew &slew){
  if (!driver_valid_ || elmore == 0.0
...
public:
  ...
  virtual void init(const LibertyLibrary *library,
		    ...
		    double rpi,
		    double c1);
  virtual void gateDelaySlew(double &delay,double &slew);
...
private:
  ...
  double ipiIceff(double t0,double dt,double ceff_time,double ceff);
...
void DmpPi::gateDelaySlew(double &delay,double &slew) {
  driver_valid_ = false;
  try {
    findDriverParamsPi();
...
void DmpPi::evalDmpEqns() {
  ...
  double y20 = y(t_vl, t0, dt, ceff);  // Match Vl.
  // F(x)
  ...
  fjac_[DmpFunc::ipi][DmpParam::dt] = (-A_ * dt + B_ * dt * exp_p1_dt - (2 * B_ / p1_) * (1.0 - exp_p1_dt)
                                      + D_ * dt * exp_p2_dt - (2 * D_ / p2_) * (1.0 - exp_p2_dt)
                                      + rd_ * ceff * (dt + dt * exp_dt_rd_ceff
		                                  - 2 * rd_ * ceff * (1.0 - exp_dt_rd_ceff)))
                                      / (rd_ * dt * dt * dt);
  fjac_[DmpFunc::ipi][DmpParam::ceff] = (2 * rd_ * ceff - dt - (2 * rd_ * ceff + dt) * exp2(-dt / (rd_ * ceff))) / (dt * dt);
...
// Eqn 13, Eqn 14.
double DmpPi::ipiIceff(double, double dt,double ceff_time,double ceff){
  double exp_p1_dt = exp2(-p1_ * ceff_time);
  double exp_p2_dt = exp2(-p2_ * ceff_time);
  double exp_dt_rd_ceff = exp2(-ceff_time / (rd_ * ceff));
  double ipi = (A_ * ceff_time + (B_ / p1_) * (1.0 - exp_p1_dt) + (D_ / p2_) * (1.0 - exp_p2_dt) / (rd_ * ceff_time * dt);
  double iceff = (rd_ * ceff * ceff_time - (rd_ * ceff) * (rd_ * ceff) * (1.0 - exp_dt_rd_ceff)) / (rd_ * ceff_time * dt);
  return ipi - iceff;
}

double DmpPi::v0(double t){
  return k0_ * (k1_ + k2_ * t + k3_ * exp2(-p1_ * t) + k4_ * exp2(-p2_ * t));
}

double DmpPi::dv0dt(double t){
  return k0_ * (k2_ - k3_ * p1_ * exp2(-p1_ * t) - k4_ * p2_ * exp2(-p2_ * t));
}

double DmpPi::vl0(double t){
  double D1 = k0_ * (k1_ - k2_ / p3_);
  double D3 = -p3_ * k0_ * k3_ / (p1_ - p3_);
  double D4 = -p3_ * k0_ * k4_ / (p2_ - p3_);
  double D5 = k0_ * (k2_ / p3_ - k1_ + p3_ * k3_ / (p1_ - p3_)+ p3_ * k4_ / (p2_ - p3_));
  return D1 + t + D3 * exp2(-p1_ * t) + D4 * exp2(-p2_ * t) + D5 * exp2(-p3_ * t);
}

double DmpPi::dvl0dt(double t){
  double D3 = -p3_ * k0_ * k3_ / (p1_ - p3_);
  double D4 = -p3_ * k0_ * k4_ / (p2_ - p3_);
  double D5 = k0_ * (k2_ / p3_ - k1_ + p3_ * k3_ / (p1_ - p3_)+ p3_ * k4_ / (p2_ - p3_));
  return 1.0 - D3 * p1_ * exp2(-p1_ * t) - D4 * p2_ * exp2(-p2_ * t) - D5 * p3_ * exp2(-p3_ * t);
}

double DmpPi::voCrossingUpperBound(){
...
double DmpOnePole::voCrossingUpperBound() {
...
void
DmpZeroC2::gateDelaySlew(double &delay,double &slew){
  try {
...
static double findRoot(void (*func)(void *state, double x, double &y, double &dy),
	void *state,
	double x1, double x2, // in range [x1,x2]
  ...
  if (y1 > 0.0) { // Swap x1/x2 so func(x1) < 0.
    double tmp = x1;
    x1 = x2;
    x2 = tmp;
  }
  ...
  for (int iter = 0; iter < max_iter; iter++) {
    ...
    if ((((root - x2) * dy - y) * ((root - x1) * dy - y) > 0.0)
	    || (abs(2.0 * y) > abs(dx_prev * dy))) {	    // Not decreasing fast enough.
    ...
    if (abs(dx) <= x_tol * abs(root)) // Converged.
      return root;
...
static void newtonRaphson(const int max_iter,
              double x[],
              const int size,
              const double x_tol,
              void (*eval)(void *state),
              ...
              double **fjac,    // J(xn)
              double *p,        // -F(xn) -> dx
              int *index,       // index of row permutation  
              double *scale)    // implicit scaling of each row
{
  for (int k = 0; k < max_iter; k++){
    eval(state);  // Compute F(xn) & J(xn)
    for (int i = 0; i < size; i++){
      p[i] = -fvec[i];       // Right-hand side of linear equations. -F(xn)
    }
    luDecomp(fjac, size, index, scale);  // LU=J(xn)
    luSolve(fjac, size, index, p);       // LU(xn-1 - xn) = p, p is solution at the end
...
void luDecomp(double **a,const int size,int *index,	double *scale){
...
void luSolve(double **a,const int size,const int *index,double b[]){
