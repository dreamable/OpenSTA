...
namespace sta
{
  ...
  MultiDrvrNet::MultiDrvrNet(VertexSet *drvrs) : dcalc_drvr_(nullptr),
                                                 drvrs_(drvrs),
  ...
  GraphDelayCalc1::GraphDelayCalc1(StaState *sta) : GraphDelayCalc(sta),
                                                    observer_(nullptr),
  ...
  FindVertexDelays::FindVertexDelays(GraphDelayCalc1 *graph_delay_calc1) : VertexVisitor(),
  ...
  FindNetDrvrs::FindNetDrvrs(PinSet &drvr_pins,
                             const Network *network,
                             const Graph *graph) : drvr_pins_(drvr_pins),
                                                   network_(network),
                                                   graph_(graph)
  {
  }
  ...
  FindNetDrvrs::operator()(const Pin *pin)
  {
    ...
    if (isLeafDriver(pin, network_) && !(vertex && vertex->isRoot()))
    ...
    if (!multi_drvr_nets_found_)
    {
      ...
      while (inst_iter->hasNext())
      {
        ...
        while (pin_iter->hasNext())
        {
          ...
          if (network_->isDriver(pin) && !multi_drvr_net_map_.hasKey(drvr_vertex))
          {
  ...
  int GraphDelayCalc1::findPortIndex(const LibertyCell *cell,
      ...
      if (wire_edge->isWire())
      {
        ...
        for (auto dcalc_ap : corners_->dcalcAnalysisPts())
        {
          ...
          for (auto rf : RiseFall::range())
          {
            ...
            // Init load vertex slew.
            if (init_load_slews && !load_vertex->slewAnnotated(rf, slew_min_max))
                                ...
                                ArcDelayCalc *arc_delay_calc)
  {
    ...
    if (from_rf && drvr_rf)
    {
      ...
      ArcDelay gate_delay;
      Slew gate_slew;
      if (multi_drvr && arc->to()->direction()->isOutput())
      ...
      const MinMax *slew_min_max = dcalc_ap->slewMinMax();
      if (delayGreater(gate_slew, drvr_slew, dcalc_ap->slewMinMax(), this) && !drvr_vertex->slewAnnotated(drvr_rf, slew_min_max) && !edge->role()->isLatchDtoQ())
        graph_->setSlew(drvr_vertex, drvr_rf, ap_index, gate_slew);
