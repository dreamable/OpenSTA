...
namespace sta
{
  ...
  GateTableModel::GateTableModel(TableModel *delay_model,
                                 ...
                                 OutputWaveforms *output_waveforms) : delay_model_(delay_model),
                             ...
                             float &cap) const
  {
    ...
    if (axis1 && axis1->variable() == TableAxisVariable::total_output_net_capacitance)
    {
      cap = axis1->axisValue(axis1->size() - 1);
      slew = findValue(library, cell, pvt, slew_model_,
                       in_slew, cap, 0.0);
    }
    else if (axis2 && axis2->variable() == TableAxisVariable::total_output_net_capacitance)
    {
      cap = axis2->axisValue(axis2->size() - 1);
      slew = findValue(library, cell, pvt, slew_model_,
                       in_slew, cap, 0.0);
    }
    else if (axis3 && axis3->variable() == TableAxisVariable::total_output_net_capacitance)
    {
                            ...
                            float related_out_cap) const
  {
    ...
    if (var == TableAxisVariable::input_transition_time || var == TableAxisVariable::input_net_transition)
      return in_slew;
  ...
  {
    TableAxisVariable var = axis->variable();
    return var == TableAxisVariable::total_output_net_capacitance || var == TableAxisVariable::input_transition_time || var == TableAxisVariable::input_net_transition || var == TableAxisVariable::related_out_total_output_net_capacitance;
  }

  ////////////////////////////////////////////////////////////////

  ReceiverModel::ReceiverModel() : capacitance_models_{{nullptr, nullptr}, {nullptr, nullptr}}
  {
  }
  ...
  {
    TableAxisVariable var = axis->variable();
    return var == TableAxisVariable::constrained_pin_transition || var == TableAxisVariable::related_pin_transition || var == TableAxisVariable::related_out_total_output_net_capacitance;
  }
  ...
  TableModel::findValue(const LibertyLibrary *library,
                        ...
                        float axis_value3) const
  {
    return table_->findValue(axis_value1, axis_value2, axis_value3) * scaleFactor(library, cell, pvt);
  }
  ...
  Table0::Table0(float value) : Table(),
                                value_(value)
  {
  }
  ...
  Table1::Table1(FloatSeq *values,
                 TableAxisPtr axis1) : Table(),
                                       values_(values),
                                       axis1_(axis1)
  {
  }

  Table1::Table1(Table1 &&table) : Table(),
  ...
  string
  Table1::reportValue(const char *result_name, const LibertyLibrary *library,
                      const LibertyCell *,
                      const Pvt *,
                      float value1,
                      const char *comment1,
                    ...
                    float) const
  {
    ...
    else
    {
      ...
      float tbl_value = (1 - dx1) * (1 - dx2) * y00 + dx1 * (1 - dx2) * y10 + dx1 * dx2 * y11 + (1 - dx1) * dx2 * y01;
      return tbl_value;
    }
  }
  ...
  Table3::Table3(FloatTable *values,
                 TableAxisPtr axis1,
                 TableAxisPtr axis2,
                 TableAxisPtr axis3) : Table2(values, axis1, axis2),
                                       axis3_(axis3)
  {
  }
                    ...
                    float axis_value3) const
  {
    ...
    float tbl_value = (1 - dx1) * (1 - dx2) * (1 - dx3) * y000 + (1 - dx1) * (1 - dx2) * dx3 * y001 + (1 - dx1) * dx2 * (1 - dx3) * y010 + (1 - dx1) * dx2 * dx3 * y011 + dx1 * (1 - dx2) * (1 - dx3) * y100 + dx1 * (1 - dx2) * dx3 * y101 + dx1 * dx2 * (1 - dx3) * y110 + dx1 * dx2 * dx3 * y111;
    return tbl_value;
  }
  ...
  TableAxis::TableAxis(TableAxisVariable variable,
                       FloatSeq *values) : variable_(variable),
                                           values_(values)
  {
  }
  ...
  {
    int max = static_cast<int>(values_->size()) - 1;
    if (!values_->empty() && value >= (*values_)[0] && value <= (*values_)[max])
    {
      int lower = -1;
      int upper = max + 1;
      bool ascend = ((*values_)[max] >= (*values_)[0]);
  ...
  OutputWaveforms::OutputWaveforms(TableAxisPtr slew_axis,
                                   ...
                                   Table1 *ref_times) : slew_axis_(slew_axis),
                                                        cap_axis_(cap_axis),
                                                        rf_(rf),
                               ...
                               float time)
  {
    ...
    float current = (1 - dx1) * (1 - dx2) * y00 + dx1 * (1 - dx2) * y10 + dx1 * dx2 * y11 + (1 - dx1) * dx2 * y01;
    return current;
  }
                               ...
                               float volt)
  {
    ...
    float time = (1 - dx1) * (1 - dx2) * y00 + dx1 * (1 - dx2) * y10 + dx1 * dx2 * y11 + (1 - dx1) * dx2 * y01;
    return time;
  }
