...
namespace sta
{
  ...
  ReduceToPi::ReduceToPi(StaState *sta) : StaState(sta),
                          ...
                          double &max_resistance)
  {
    ...
    y1 = dwn_cap = parasitics_->nodeGndCap(node, ap) + coupling_cap * coupling_cap_multiplier_ + pinCapacitance(node);
    y2 = y3 = 0.0;
    ...
    while (device_iter2->hasNext())
    {
      ...
      if (parasitics_->isResistor(device) && !isLoopResistor(device))
      {
  ...
  ReduceToPiElmore::ReduceToPiElmore(StaState *sta) : ReduceToPi(sta)
  {
  }
                                    ...
                                    const ParasiticAnalysisPt *ap)
  {
    ...
    while (device_iter->hasNext())
    {
      ...
      {
        ParasiticNode *onode = parasitics_->otherNode(device, node);
        if (device != from_res && !isVisited(onode) && !isLoopResistor(device))
        {
          float r = parasitics_->value(device, ap);
          double onode_elmore = elmore + r * downstreamCap(onode);
          reduceElmoreDfs(drvr_pin, onode, device, onode_elmore, pi_elmore, ap);
        }
      }
    }
    delete device_iter;
  ...
  ReduceToPiPoleResidue2::ReduceToPiPoleResidue2(StaState *sta) : ReduceToPi(sta),
                                                                  moments_(nullptr)
  {
  }
                                             ...
                                             const ParasiticAnalysisPt *ap)
  {
    ...
    while (device_iter->hasNext())
    {
      ...
      if (parasitics_->isResistor(device))
      {
        ParasiticNode *onode = parasitics_->otherNode(device, node);
        // One commercial extractor creates resistors with identical from/to nodes.
        if (onode != node && device != from_res && !isVisited(onode) && !isLoopResistor(device))
        {
    ...
    delete device_iter;
    double cap = parasitics_->nodeGndCap(node, ap) + coupling_cap * coupling_cap_multiplier_ + pinCapacitance(node);
